...
**TODO: add zip code, and geography**
...
```{r}
# age - relationship looks curved
ggplot(d , aes(x=factor(age), y=retirement_score, col = as.factor(treatment_ind))) + geom_boxplot() + xlab("Age") + ylab("Retirement Score") + labs(col = "treatment") + theme(axis.text.x = element_text(angle=75, hjust=1))

# gender
ggplot(d %>%
         filter(., gender != ""), aes(x=factor(gender), y=retirement_score, col = as.factor(treatment_ind))) + geom_boxplot() + xlab("Gender") + ylab("Retirement Score") + labs(col = "treatment") + theme(axis.text.x = element_text(angle=75, hjust=1))

#education - not a compelling candidate
par(mar=c(10,3,1,1))
ggplot(d , aes(x=factor(highest_education, highest_education_order), y=retirement_score, col = as.factor(treatment_ind))) + geom_boxplot() + xlab("Highest Education") + ylab("Retirement Score") + labs(col = "treatment") + theme(axis.text.x = element_text(angle=75, hjust=1))
```
...
For our respondents’ outlook on employment opportunities, we hypothesized that in addition to the treatment, the covariates representing political preference, income level, highest level of education, age, gender and community (rural, suburban and urban); these were informed both by intuition and our exploration of the data. After running a log-likelihood ratio test on the above regression, we found that only political preference, income, and community significantly improved the fit of the model. Our estimated treatment effect however, while consistently negative, is not statistically significant, and neither is the treatment effect on specific political preference categories (see Table 1). Unsurprisingly, the _Right/Republican_ preference has an estimated effect of over 15 points with a p-value less than 0.01, while high income also has a slight positive effect. More interesting, _Suburban_ (and to a lesser extent Rural) has a negative relative to _Urban_ respondents.
...
lm.emp.2 <- lm(employment_score ~ treatment_ind + political_party + income + age + community, data = d)
lm.emp.3 <- lm(employment_score ~ treatment_ind + treatment_ind*political_party + income + age + community, data = d)
...
While our prompt on the value of education toward preparing students for the workforce did not have a Trump treatment, the variable as expected was indistinguishable from zero (see Table 2). However, to our surprise, of our conjectured covariates that would contribute to the fit of the model, _Right/Republican_ had a positive effect relative to the other categories and moderately significant at the 95% confidence level. _Female_ also had a modest positive effect at a similar significance level. Interestingly, level of education did _not_ have a significant effect on the subjects’ estimated _value_ of education; similarly, level of income did not have a strong effect and did not improve the model.
...
lm.ed.2 <- lm(education_score ~ treatment_ind + highest_education + political_party + gender + income,
              data = d)
...
Similarly to the estimated treatment effect on employment outlook, we see a slight positive but statistically insignificant effect on retirement savings optimism (see Table 3). Of the covariates, we again see that _Right/Republican_ is more optimistic, suggesting our topics are unfortunately more influenced by politics than we had hoped; correspondingly, _Left/Liberal_ are also significantly lower than their _Middle/Independent_ counterparts. More surprising, however, is that female subjects give a significantly lower level of optimism than males, which both high income and high level of education have modest positive effects. Like employment outlook, there appears to be no heterogeneous effects by political preference.
...
lm.ret.2 <- lm(retirement_score ~ treatment_ind + 
                 political_party + gender + age + income + highest_education , data = d)
lm.ret.3 <- lm(retirement_score ~ treatment_ind + 
                 treatment_ind*political_party + gender + age + income + highest_education, data = d)
...
Unlike the prior topics which are more economic and perhaps present in day-to-day life, the risk of cybersecurity produced some interesting relationships (see Table 4). While _Right/Republican_ has a very significant positive effect than all others (i.e. perceiving the risk to be higher), the perceive the risk to be _lower_ when in the treatment group, for a modest confidence level of 95%. Meanwhile, older respondents have a higher estimated score, while income, age and education did not improve the estimated model.
