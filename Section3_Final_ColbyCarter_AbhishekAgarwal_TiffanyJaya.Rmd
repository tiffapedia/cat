...
freq_responseid <- d %>% group_by(ResponseId) %>% summarize(freq = sum(n()))
...
d %>% filter(., country == "United States") %>% count(.)
# how many subjects are from outside the United States?
d %>% filter(., country != "United States") %>% count(.)
```

```{r}
# filter out USA subjects
subjects.usa <- d %>% filter(., country == "United States")
```
