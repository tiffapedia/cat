...
d$income_group <- relevel(d$income, "medium")
d$age_group <- relevel(d$age, "middle-aged adult")
```
...
gg <- ggplot(states)
ggplot(map.df, aes(x=d$LocationLatitude, y=d$LocationLongitude))+
...
stargazer(lm.ed.1, lm.ed.2, title = "Value on Education", type = "text",
...
Unlike the prior topics which are more economic and perhaps present in day-to-day life, the risk of cybersecurity produced some interesting relationships (see Table 4). While the covariate _Right/Republican_ has a significantly more positive effect than all others (i.e. perceiving the risk to be higher), they perceived the risk to be _lower_ when in the treatment group, a modest heterogeneous treatment effect at the 5% significance level. Meanwhile, older respondents have a higher estimated score, while income, age and education did not improve the estimated model.
