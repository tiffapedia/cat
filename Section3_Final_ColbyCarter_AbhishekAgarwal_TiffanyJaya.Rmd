...
country_names <- mapply(
  function(lat, lng) {
    suppressWarnings(try(GNcountryCode(lat, lng)$countryName, silent = TRUE))
  }, d$LocationLatitude, d$LocationLongitude)
```

```{r}
as.data.frame(table(country_names))
```

TODO: filter out just the USA
...
Getting the USA (or any other country)'s latlong
