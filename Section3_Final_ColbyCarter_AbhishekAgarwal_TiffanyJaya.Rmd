...
A related survey was conducted recently to determine the level of optimism American workers have in employment outlook a year after President Trump's election[3]. It found that men were estimated twice as likely as women to be more optimistic, while there were also differences by race, education and geography. In our study, we control for these factors while considering the possibility of heterogeneous treatment effects. 
...
Education is the only prompt that does not contain any Trump treatment in either subject group, which serves as both a check that our treatment was indeed chosen randomly and to avoid tipping the purpose of the survey. With each prompt, the respondent is given a minimum of 40 seconds to read the text and then answer the question, which is a sliding scale from 0 to 100; we use this wide scale in order to achieve some variance in the distribution of responses. The flow of the survey as well as an example of the scale are shown in Figure 1 and Figure 2-3 respectively. 

![Survey Flow](images/survey_flow.png)

![Scale used for employment, education, and retirement prompt](images/optimistic_scale.png)

![Scale used for cybersecurity prompt](images/threat_scale.png)

## Randomization Engineering

To collect a broad sample of the American population, we turned to Mechanical Turk to select our subjects. We used Qualtrics to assign subjects to their respective control and treatment groups as shown in Figure 4. 

![Random Assignment](images/random_assignment.png)
...
In addition, we filtered out subjects who did not complete the survey (including those who did not provide their consent) as well as subjects who took the survey more than once, which left us with 557 subjects with 262 in control and 295 in treatment.   
...
gg <- gg + geom_map(data=geomap1, map=us,
